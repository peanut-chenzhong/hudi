...
case class CreateHoodieTableAsSelectCommand(
  ...
  override def run(sparkSession: SparkSession): Seq[Row] = {
    ...
    if (hasQueryAsProp) {
      throw HoodieAnalysisExceptionUtils.analysisException("Not support CTAS for the ro/rt table")
    }
    ...
    if (sessionState.catalog.tableExists(qualifiedTableIdentifier)) {
      ...
      if (mode == SaveMode.ErrorIfExists) {
        throw HoodieAnalysisExceptionUtils.analysisException(s"Table $tableName already exists. You need to drop it first.")
      }
